...
@sample1 = (sortLetters(@sample1[0]), @sample1[1]);
@sample2 = (sortLetters(@sample2[0]), @sample2[1]);
@sample3 = (sortLetters(@sample3[0]), @sample3[1]);
@sample4 = (sortLetters(@sample4[0]), @sample4[1]);
is @sample1[0][^5].join, 'abxyz';
is @sample2[0][^5].join, 'abcde';
is @sample3[0][^5].join, 'oarel';
is @sample4[0][^5].join, 'loart';

is isReal(@sample1[0], @sample1[1]), True;
is isReal(@sample2[0], @sample2[1]), True;
is isReal(@sample3[0], @sample3[1]), True;
is isReal(@sample4[0], @sample4[1]), False;

my @decoded = "static/4.txt".IO.lines.map({ decode($_);});
my @sorted = @decoded.map: {(sortLetters($_[0]), $_[1])};
my @seen = @sorted ==> map { isReal($_) } ==> grep;
say @seen.elems;
